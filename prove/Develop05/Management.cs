...
public void SaveGoalsToJson(string filename)
{
    try
    {
        // Build the JSON array manually using StringBuilder
        var jsonBuilder = new StringBuilder();
        jsonBuilder.AppendLine("[");
        
        for (int i = 0; i < _goals.Count; i++)
        {
            // Call the ToJson method for each goal and append it to the builder
            Console.WriteLine(_goals[i].ToJson());

            jsonBuilder.Append(_goals[i].ToJson());
            if (i < _goals.Count - 1) // Add a comma between JSON objects, except for the last one
            {
                jsonBuilder.AppendLine(",");
            }
        }
